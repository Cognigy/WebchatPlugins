...
const SignInWithMicrosoft = (props) => {
    ...
    const onClick = () => {
        const { clientId, redirectUri, scope } = message.data._plugin;

        window.open(`https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURI(redirectUri)}&response_mode=query&scope=${scope}`)
    }
