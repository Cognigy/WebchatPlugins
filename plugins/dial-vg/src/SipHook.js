...
import { VIVA_WSS_URI } from '../config';
...
export default function useSip() {
	...
	function AddSipSessionEventHandlers(ua, session) {
		...
		session.on('change', () => {
			/* something changed in the session */
	...
	useEffect(() => {
		if (sipClientDetails) {
			console.log({ sipClientDetails }, 'sipHook: setting active client');
			sipClientDetails.registered = false;
			const ua = new SipClient(sipClientDetails, settings);
			ua.start();
			addUAEventListeners(ua);
			sclient.current = ua;
		}
		return () => {
			/* clean up by unregistering previous client */
